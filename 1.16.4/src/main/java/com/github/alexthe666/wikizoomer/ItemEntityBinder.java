...
public class ItemEntityBinder extends Item {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if(!isEntityBound(stack)){
            tooltip.add(new TranslationTextComponent("item.wikizoomer.entity_binder.desc").func_240699_a_(TextFormatting.GRAY));
        }
        if (stack.getTag() != null) {
            ...
            if (entity instanceof CompoundNBT) {
                ...
                if(optional.isPresent()){
                    ITextComponent untranslated = isPlayer ? new TranslationTextComponent("entity.player.name").func_240699_a_(TextFormatting.GRAY) : optional.get().getName();
