...
public class TileEntityEntityZoomer extends TileEntityZoomerBase {
    ...
    public TileEntityEntityZoomer(BlockPos pos, BlockState state) {
        super(WikiZoomerMod.ENTITY_ZOOMER_TE, pos, state);
    }

    public static void tick(Level level, BlockPos pos, BlockState state, TileEntityEntityZoomer entity) {
        entity.baseTick();
        if (entity.getItem(0).getItem() != WikiZoomerMod.ENTITY_BINDER_ITEM) {
    ...
    public Entity getCachedEntity(){
        if(this.getItem(0).getItem() == WikiZoomerMod.ENTITY_BINDER_ITEM){
    ...
    protected Component getDefaultName() {
        return new TranslatableComponent("block.wikizoomer.entity_zoomer");
    }
}
